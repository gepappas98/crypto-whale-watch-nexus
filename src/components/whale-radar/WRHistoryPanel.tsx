 ...
 *  scan-history read API since early on — getScanSessions(), getTokenHistory(),
 *  getTopThreats() — backed by real server/routes/scans.ts endpoints and a
 *  populated scan_coins/scan_sessions table (saveScan() has been called from
 *  useMarketData.ts on every scan all along). Nothing ever called the read
 *  side. This panel is that missing consumer.
 *
 *  Fetched on-demand (tab open / refresh click), not polled — this is
 *  historical data, it doesn't change fast enough to justify a live loop,
 *  and it would just be extra backend load for no benefit.
 * ═══════════════════════════════════════════════════════════════════════════ */
import { useCallback, useEffect, useState } from 'react';
import { getScanSessions, getTokenHistory, getTopThreats, type ScanSession, type HistoricalCoin, type TopThreatRow } from '@/lib/db';
    ...
    <div className="border-b border-wr-border">
      ...
      <div className="max-h-32 overflow-y-auto scrollbar-thin">
        {sessions.length === 0 ? (
          ...
          sessions.map((s) => (
            <div key={s.id} className="px-3 py-1 border-b border-wr-border/30 grid grid-cols-[1fr_auto_auto] gap-2 items-center text-[8px]">
              <span className="text-wr-muted">{timeAgo(s.scanned_at)}</span>
