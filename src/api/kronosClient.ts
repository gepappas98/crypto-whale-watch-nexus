const KRONOS_BASE = import.meta.env.VITE_KRONOS_URL || "http://localhost:8000";
...
export type Signal = "STRONG_BULL" | "BULLISH" | "NEUTRAL" | "BEARISH" | "STRONG_BEAR";
...
export async function fetchForecast(req: ForecastRequest): Promise<ForecastResponse> {
  ...
  if (!res.ok) {
    ...
    throw new Error(err.detail ?? "Kronos request failed");
  }
  return res.json();
}
...
export async function checkHealth(): Promise<boolean> {
  try {
    const res = await fetch(`${KRONOS_BASE}/health`, { signal: AbortSignal.timeout(5000) });
