...
- **`ProtectionOptimizerPanel`'s Apply button had its own copy of `applyBestCandidate()`'s merge logic** (v9.16 follow-up, no version bump — too small): a fresh dead-export sweep after v9.16 turned up mostly false positives (functions superseded by better inline implementations, e.g. `getSymbolPerformance()`/`analyzeBatchRisk()` — both fine as-is, just not the path the UI ended up using) plus this one real, small DRY risk: the panel rebuilt the exact same `ProtectionConfig` merge that `protectionOptimizer.ts`'s own `applyBestCandidate()` already does, as two copies that could silently drift if either changed. Now calls the shared function instead.
- **The mlConfidence floor now has real provenance for pairs with history** (v9.16): closes the item the v9.14/v9.15 changelogs kept flagging as open. The original plan floated there was "HMAC-sign the client's number" — turned out to be the wrong fix on inspection: a signature can prove a number wasn't tampered with in transit, but it can't make an unverifiable number (computed by a model that trains on browser localStorage the server has never seen) suddenly trustworthy. The real fix is `server/services/signalConfidence.ts`: a win-rate computed from `signal_outcomes`, the server's own Postgres history of past signal fires with filled-in 4h outcomes — nothing the client sends. `/enter` now prefers this when there are at least 8 filled-outcome fires for the pair (`MIN_SAMPLES`); the client-reported `mlConfidence` is downgraded to audit-only metadata in that case, still logged in the trade ledger but never used to gate. Only when the server has no track record yet for a pair does this fall back to the older, weaker client-reported-number check — and the 422 error response says so explicitly (`confidenceSource: 'server' | 'client' | 'none'`), so it's never silently ambiguous which check actually ran. This is a coarser signal than the client's per-feature logistic regression (a win rate, not a full feature vector) but it's the first one in this feature that the client genuinely cannot lie about.
...
Both previously-open roadmap items are closed as of v9.7/v9.8 — see ✅ Done below. v9.9-v9.16 were separately-found gaps, not roadmap items: v9.9-v9.11 were "logic exists, no UI consumer," v9.12 was a fake-output variant of the same pattern, v9.13 was the inverse (a documented, UI-wired feature with no server behind it at all), v9.14 was a hardening pass on v9.13's own gating logic, v9.15 found the same "documented but never built" pattern as v9.13 twice more (push notifications, the install prompt), and v9.16 closed the mlConfidence provenance item v9.14/v9.15 had flagged. One real, currently-open item: none of Whale Radar's Supabase Edge Functions (`nexus-bot-proxy`, `push-proxy`, and the rest) have per-user authentication — only the public anon key — because the app has no login system at all; adding one is a real feature, not a patch. Otherwise, next candidates would need a new gap identified (e.g. via a fresh pass over `freqtrade-develop` or a fresh look at what's stubbed vs. real in Nexus/Council).

✅ Done (were previously listed here as roadmap items):
- **Strategy Trader's server side actually exists now, with real (not client-reported) confidence gating** — see [What's New](#-whats-new) above.
