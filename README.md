# 🐋 Whale Radar (crypto-whale-watch-nexus) — v9.27

![Version](https://img.shields.io/badge/version-9.27-blue)
...
- Real market breadth beyond the current watchlist-scoped proxy (`breadthSignal` in `signals.ts` reads the live scanner's coin list, which is whatever the user happens to be tracking, not the full market).

**Open tuning question, deliberately not resolved unilaterally** (see v9.25's changelog entry): `PERSISTENCE_SNAPSHOTS = 3` at a 5-minute poll interval confirms a regime after 15 minutes, looser than this doc's original 5min → 30min → 2h persistence idea. Left as a decision to make deliberately, not changed as a side effect of an audit.
