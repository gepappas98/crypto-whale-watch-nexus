# 🐋 Whale Radar (crypto-whale-watch-nexus) — v9.24

![Version](https://img.shields.io/badge/version-9.24-blue)
...
## 🧭 Strategic Direction (proposed pivot, not yet built)

A deliberate reframing of what this project is *for*, decided but not yet implemented — recorded here so it governs prioritization from this point on, and so the reasoning survives past this session.
...
Concretely: not "BTC will go up 50%," but "7 independent signals that historically precede a broader bullish transition are now present; regime moved NEUTRAL → EARLY BULL WATCH." Confidence over prediction, transition-detection over price-target-calling.

**What already exists that this can be built from** (verified against source, not assumed): whale flow, aggressive buy/sell flow, orderbook pressure, funding/OI, technical indicators, Fear&Greed + BTC/ETH dominance (`get-market-sentiment.ts`), top movers, exchange comparisons, per-signal ML confidence (`mlScoring.ts`), historical signal performance (`pairPerformance.ts`), wallet activity, a 6-desk AI Council, multi-channel alerts with an existing circuit-breaker (`alertCooldown.ts`). All genuinely real, all currently **per-asset** — none of it combined into a market-wide read today.

**What's genuinely new, doesn't exist yet:**
- A **Regime Engine**: classifies the whole market into `BEAR → RECOVERY → EARLY BULL → BULL → LATE BULL → DISTRIBUTION`, not just up/down.
- **Market breadth**: % of tracked alts participating in the move, not just BTC/ETH.
- A **Correlation Engine**: counts how many independent signals currently agree (1 = noise, 3 = watch, 6+ = early opportunity, 8+ = confirmation) — this is what actually suppresses false positives.
- **Signal persistence**: a detected condition must hold across multiple re-checks (5min → 30min → 2h) before it's allowed to raise alert severity, not fire on a single spike.
- **Regime acceleration**: rate of change of the regime score (42 → 61 → 74), not just its current value.
- A restructured Council: 🐂 Bull / 🐻 Bear / 🐋 Flow / 📊 Quant / 🧠 **Regime** (new) / 🛡 Risk, answering "what regime are we in" and "what would invalidate this," not just "is this coin bullish."
- **"Why am I getting this alert"** — every alert names its specific trigger conditions, not just a score.
- A **decision-outcome loop**: "Mark as Reviewed" / "I Bought" buttons that log the alert → the user's actual action → later P/L, so the ML can eventually learn *which alert types were actually useful to this specific user* — not just per-signal confidence, but per-alert-combination usefulness.
- A homepage that leads with global regime state, not a trading terminal.
...
**Priority order for the new direction** (P0 blocks everything downstream of it — nothing above is buildable before the Regime Engine's actual scoring logic is designed):
- P0: Regime Engine + Early Bull detection + signal persistence + correlation counting + alert reliability/false-positive suppression.
- P1: Market breadth, stablecoin/liquidity flow, BTC/ETH relative strength, whale accumulation/distribution regime classification, derivatives regime, historical signal evaluation, personal watchlist scoring.
- P2: Council explanation depth, personal outcome learning, homepage redesign, the execution features above (now optional, not core).

**Not yet decided, needs input before P0 code gets written**: the actual signal list (which 15-25 inputs feed the regime score), their weights, and the score thresholds that separate NEUTRAL/EARLY BULL/BULL — these are inherently tunable judgment calls, not something to hardcode from a first pass. A grounded starting proposal (using only signals that already exist as real data — no invented ones like "stablecoin flow" until that's actually built): BTC trend vs. 50/200-EMA, whale buy/sell ratio (existing whale-feed), OI rate-of-change, funding rate level, Fear&Greed level + its rate-of-change, BTC dominance trend, aggressive buy/sell flow ratio, and `mlScoring.ts`'s existing per-symbol confidence aggregated across the watchlist as a breadth proxy until real breadth is built. Needs review/tuning, not a unilateral first guess, before P0 implementation starts.



Both previously-open roadmap items are closed as of v9.7/v9.8 — see ✅ Done below. v9.9-v9.12 were separately-found gaps (mostly "logic exists, no UI consumer"; v9.12 was a fake-output variant of the same pattern), not roadmap items. v9.13/v9.14 were AI-discoverability additions (crawler-facing static shells, then an MCP tool server) rather than gap fixes. v9.15-v9.19 were bug-fix passes on those additions (Reddit/news upstream blocking, an MCP audit's findings, and a caching/upstream-load cleanup) rather than new gaps or roadmap items. v9.20 closed the push-notifications item that had been flagged as a real next candidate since v9.10. v9.21 was another small "no UI consumer" find (`getScanCoins()`), same family as v9.6/v9.9-v9.12. v9.22 closed the Solana pre-pump-transfer-detection gap's *transfer-detection* half — the CEX-address-list half is deliberately left as a manual step; see v9.22 in [What's New](#-whats-new) for why a hardcoded guess wasn't the right fix. v9.23 wasn't a new gap at all — a fresh pass over `src/lib/mcp/` (the area flagged as a next-candidate source) found v9.18's `list_council_decisions` exact-match fix had landed in an orphaned, never-imported duplicate file instead of the live one, so the substring-match bug was still shipping; corrected at the actual source, dead duplicate files removed. v9.24 swept `src/lib/nexus/` next: closed two more "no UI consumer" gaps in the protection engine (bulk lock-clearing, current-config visibility + reset), and traced `getManifest()`/`describeManifest()` — the one export in that sweep still uncalled — to a real architectural blocker (browser-localStorage-only state vs. a Supabase-Edge-Function MCP server with no path to it) rather than force-wiring something that wouldn't reach its intended audience anyway. One small real gap remains on record, not force-closed: `triggerBackgroundSync()` (`src/lib/pwa.ts`, no `sync` handler in `public/sw.js` and no offline-write queue to attach one to). A comparison against [PolyBot](https://github.com/cryptuon/polybot) (a more mature agent-trading framework) surfaced four genuinely new candidates around AI/Council-to-execution safety, none built yet — see 🔜 Planned below; the most directly buildable of the four is the human-approval queue, since it sits on top of plumbing (`strategyTraderBridge.ts`) that already exists. Otherwise nothing outstanding; next candidates would need a new gap identified (e.g. via a fresh pass over `freqtrade-develop`, `src/lib/council/`, or `server/services/`), a verified Solana CEX-address list supplied for the v9.22 item, an actual architecture change for the v9.24 manifest item, or work starting on one of the 🔜 Planned items below.
