# 🐋 Whale Radar (crypto-whale-watch-nexus) — v9.9
...
Both previously-open roadmap items are closed as of v9.7/v9.8 — see ✅ Done below. v9.9's scan-history fix was a separately-found gap (same "logic exists, no UI consumer" pattern), not a roadmap item. Nothing currently outstanding; next candidates would need a new gap identified (e.g. via a fresh pass over `freqtrade-develop` or a fresh look at what's stubbed vs. real in Nexus/Council).
