# 🐋 Whale Radar (crypto-whale-watch-nexus) — v9.23

![Version](https://img.shields.io/badge/version-9.23-blue)
...
Both previously-open roadmap items are closed as of v9.7/v9.8 — see ✅ Done below. v9.9-v9.12 were separately-found gaps (mostly "logic exists, no UI consumer"; v9.12 was a fake-output variant of the same pattern), not roadmap items. v9.13/v9.14 were AI-discoverability additions (crawler-facing static shells, then an MCP tool server) rather than gap fixes. v9.15-v9.19 were bug-fix passes on those additions (Reddit/news upstream blocking, an MCP audit's findings, and a caching/upstream-load cleanup) rather than new gaps or roadmap items. v9.20 closed the push-notifications item that had been flagged as a real next candidate since v9.10. v9.21 was another small "no UI consumer" find (`getScanCoins()`), same family as v9.6/v9.9-v9.12. v9.22 closed the Solana pre-pump-transfer-detection gap's *transfer-detection* half — the CEX-address-list half is deliberately left as a manual step; see v9.22 in [What's New](#-whats-new) for why a hardcoded guess wasn't the right fix. v9.23 wasn't a new gap at all — a fresh pass over `src/lib/mcp/` (the area flagged as a next-candidate source) found v9.18's `list_council_decisions` exact-match fix had landed in an orphaned, never-imported duplicate file instead of the live one, so the substring-match bug was still shipping; corrected at the actual source, dead duplicate files removed. One small real gap remains on record, not force-closed: `triggerBackgroundSync()` (`src/lib/pwa.ts`, no `sync` handler in `public/sw.js` and no offline-write queue to attach one to). Otherwise nothing outstanding; next candidates would need a new gap identified (e.g. via a fresh pass over `freqtrade-develop` or a fresh look at what's stubbed vs. real in Nexus/Council), or a verified Solana CEX-address list supplied for the v9.22 item above.
