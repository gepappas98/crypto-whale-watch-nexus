...
Both previously-open roadmap items are closed as of v9.7/v9.8 — see ✅ Done below. v9.9-v9.13 were separately-found gaps, not roadmap items: v9.9-v9.11 were "logic exists, no UI consumer," v9.12 was a fake-output variant of the same pattern, and v9.13 was the inverse — a documented, UI-wired feature with no server behind it at all. Nothing currently outstanding; `subscribeToPush()`/`triggerBackgroundSync()` (see [What's New](#-whats-new)) are a real next candidate but need backend work (VAPID keys, a push-sending endpoint) first. Otherwise, next candidates would need a new gap identified (e.g. via a fresh pass over `freqtrade-develop` or a fresh look at what's stubbed vs. real in Nexus/Council).
