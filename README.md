# 🐋 Whale Radar (crypto-whale-watch-nexus) — v9.18

![Version](https://img.shields.io/badge/version-9.18-blue)
...
- **MCP tools: fixed the real BTC/ETH bug, added upstream caching, corrected two indicators, tightened a filter** (v9.18): an external audit of `src/lib/mcp/` found a critical, several real, and a couple of speculative issues — worked through the real ones:
  - **Critical, actually fixed now**: `binance.ts`'s `toPair()` had `/(USDT|USDC|BUSD|BTC|ETH)$/.test(raw)` — which matched `"BTC"` and `"ETH"` against *themselves* (the whole string is the suffix), so a bare `BTC` or `ETH` query hit Binance as `symbol=BTC`/`symbol=ETH` instead of `BTCUSDT`/`ETHUSDT` — Binance has no such pair, so `get_market_snapshot`, `get_whale_trades`, `get_orderbook_pressure`, `get_price_history`, `get_trade_flow`, `get_technical_indicators`, and `get_funding_and_open_interest` all failed on the two most commonly requested assets. A prior attempt at this fix hadn't actually reached `src/lib/mcp/binance.ts` (the real source file) — only the *auto-generated* `supabase/functions/mcp/index.ts` bundle exists, and it explicitly says not to hand-edit it (regenerated from `src/lib/mcp/` by the `@lovable.dev/mcp-js` Vite plugin on every `npm run build` — verified: rebuilding now regenerates it with the fix). The new check requires a *non-empty* base-asset prefix before the quote suffix, so `BTCUSDT`/`ETHBTC`/`SOLBTC` still pass through unchanged — only a bare quote-currency-lookalike with nothing in front gets `USDT` appended.
  - **Upstream load**: `get_top_movers`, `search_assets` (both fetch Binance's entire 24hr ticker dataset) and `get_hyperliquid_market` (fetches Hyperliquid's entire market universe) now share a new `cached()` helper in `binance.ts` — an 8s in-memory cache scoped to one warm Edge Function instance (same limitation as `trading-bridge`'s `redditToken` cache: not distributed, resets on a cold instance). This is real protection against the audit's "upstream API load" concern — it bounds how often the expensive request actually goes out, independent of how many MCP calls arrive. What it explicitly is **not**: per-client rate limiting. `defineTool`'s handler only receives the tool's structured input — no request object, no caller IP — so there's no caller identity available at this layer to throttle against; that would need a gateway/reverse-proxy in front of the Supabase Edge Function, not something a tool handler can do. Documented as a real constraint, not silently skipped.
...
`src/lib/mcp/binance.ts` and `supabase.ts` are the shared fetch helpers (10s timeout + abort, an 8s in-memory `cached()` wrapper for the two full-dataset Binance/Hyperliquid calls, and env-var resolution across `SUPABASE_URL`/`VITE_SUPABASE_URL` naming, matching the rest of the app's Supabase config tolerance). `list_council_decisions` reads with the `anon` key only — RLS-gated, no write access. `supabase/functions/mcp/index.ts` is an **auto-generated bundle** (built by the `@lovable.dev/mcp-js` Vite plugin from `src/lib/mcp/` on every `npm run build`) — never hand-edit it, edit the source files under `src/lib/mcp/` and rebuild.
