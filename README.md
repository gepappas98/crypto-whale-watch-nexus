# 🐋 Whale Radar (crypto-whale-watch-nexus) — v9.21

![Version](https://img.shields.io/badge/version-9.21-blue)
...
Both previously-open roadmap items are closed as of v9.7/v9.8 — see ✅ Done below. v9.9-v9.12 were separately-found gaps (mostly "logic exists, no UI consumer"; v9.12 was a fake-output variant of the same pattern), not roadmap items. v9.13/v9.14 were AI-discoverability additions (crawler-facing static shells, then an MCP tool server) rather than gap fixes. v9.15-v9.19 were bug-fix passes on those additions (Reddit/news upstream blocking, an MCP audit's findings, and a caching/upstream-load cleanup) rather than new gaps or roadmap items. v9.20 closed the push-notifications item that had been flagged as a real next candidate since v9.10. v9.21 was another small "no UI consumer" find (`getScanCoins()`), same family as v9.6/v9.9-v9.12. Two small real gaps are on record but deliberately not force-closed: `triggerBackgroundSync()` (`src/lib/pwa.ts`, no `sync` handler in `public/sw.js` and no offline-write queue to attach one to), and Solana-chain pre-pump-transfer detection (`insiderRiskApi.ts`'s `BirdeyeService`, needs a verified Solana CEX-address list this repo doesn't have — see v9.21 in [What's New](#-whats-new)). Otherwise nothing outstanding; next candidates would need a new gap identified (e.g. via a fresh pass over `freqtrade-develop` or a fresh look at what's stubbed vs. real in Nexus/Council).
